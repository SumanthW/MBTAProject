...
FROM (
  SELECT s.name, td.station_id, EXTRACT(YEAR FROM t.swipe_time) AS swipe_year, SUM(t.value) AS total_revenue,
         ROW_NUMBER() OVER (PARTITION BY EXTRACT(YEAR FROM t.swipe_time) ORDER BY SUM(t.value) DESC) AS revenue_rank
  FROM transaction t
  JOIN transaction_device td ON td.transaction_device_id = t.transaction_device_id
