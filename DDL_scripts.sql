...
SELECT TO_CHAR(swipe_time, 'YYYY') AS TRANSACTION_YEAR, SUM(value) AS TOTAL_REVENUE
FROM TRANSACTION
GROUP BY TO_CHAR(swipe_time, 'YYYY')
ORDER BY TO_CHAR(swipe_time, 'YYYY');

--Revenue generated by each transit over years
DECLARE
  CURSOR c_transit IS
    SELECT transit_id, name
    FROM transit;
  
  v_transit_id transit.transit_id%TYPE;
  v_transit_name transit.name%TYPE;
  v_revenue NUMBER;
BEGIN
  FOR rec IN c_transit LOOP
    v_transit_id := rec.transit_id;
    v_transit_name := rec.name;
    
    -- Calculate revenue for each transit
    SELECT SUM(value)
    INTO v_revenue
    FROM transaction t
    JOIN wallet w ON t.wallet_id = w.wallet_id
    JOIN card c ON w.wallet_id = c.wallet_id
    WHERE c.card_id IN (
      SELECT card_id
      FROM card
      WHERE wallet_id IN (
        SELECT wallet_id
        FROM wallet
        WHERE wallet_type = 'Card'
        )
      )
    AND t.transaction_type IN ('Ride', 'Pass')
    AND EXISTS (
      SELECT 1
      FROM line l
      WHERE l.transit_id = v_transit_id
      AND l.line_id = (
        SELECT line_id
        FROM transaction_device td
        WHERE td.transaction_device_id = t.transaction_device_id
        )
      );
    
    -- Output results for each transit
    IF v_revenue IS NOT NULL THEN
      DBMS_OUTPUT.PUT_LINE(v_transit_name || ' generated ' || TO_CHAR(v_revenue, 'FM999G999G999D99') || ' in revenue.');
    ELSE
      DBMS_OUTPUT.PUT_LINE(v_transit_name || ' generated no revenue.');
    END IF;
  END LOOP;
END;
/
