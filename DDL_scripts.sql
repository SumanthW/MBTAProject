...
CREATE TABLE TRANSACTION(transaction_id number GENERATED BY DEFAULT AS IDENTITY, transaction_type varchar(20), swipe_time timestamp, wallet_id number not null, value number, transaction_device_id number, PRIMARY KEY(transaction_id), FOREIGN KEY (wallet_id) REFERENCES wallet(wallet_id), FOREIGN KEY (transaction_device_id) REFERENCES TRANSACTION_DEVICE(transaction_device_id), CONSTRAINT transaction_type_option CHECK (transaction_type in ('Pass','Ride','Card_Balance')));
